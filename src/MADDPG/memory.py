...
# obs,action -> concatenate all agents obs/action together
# 
# terminals1 ->term in ||next|| state, need no more proceeding

class Memory(object):
    def __init__(self, num_agents, limit, action_shape, observation_shape):
        self.limit = limit
        self._next_entry = 0
        self._nb_entries = 0
        self.use_cuda = False
        ...
        self.data_buffer = {}
        self.data_buffer['obs0'      ] = [torch.zeros((limit,) + (num_agents*shape[0],)) for shape in observation_shape]
        self.data_buffer['obs1'      ] = [torch.zeros((limit,) + (num_agents*shape[0],)) for shape in observation_shape]
        self.data_buffer['actions'   ] = torch.zeros((limit,) + (num_agents*action_shape[0],))
        self.data_buffer['rewards'   ] = torch.zeros((limit,num_agents))
        self.data_buffer['terminals1'] = torch.zeros((limit,num_agents))
    ...
    def append(self, obs0, action, reward, obs1, terminal1):
        for item_idx in range(len(obs0)):
            #print(self.data_buffer['obs0'][item_idx][self._next_entry])
            #print(torch.as_tensor(obs0[item_idx]))
